"use client";

import { useMemo, useState } from "react";
import Link from "next/link";
import { Product, fromPrice, hasPricedOptions } from "@/lib/types";
import { formatPrice, discountPercent } from "@/lib/format";
import { SafeImage } from "@/components/SafeImage";
import { coverImageForProduct } from "@/lib/product-images";
import { Icon } from "@/components/Icons";

type Chip = {
  id: string;
  label: string;
  filter: (p: Product) => boolean;
};

const CHIPS: Chip[] = [
  { id: "all", label: "All picks", filter: () => true },
  {
    id: "deals",
    label: "🔥 Tech Deals",
    filter: (p) =>
      Boolean(p.hotDeal || (p.compareAtPrice && p.compareAtPrice > p.price))
  },
  {
    id: "phones",
    label: "Phones",
    filter: (p) => p.categorySlug === "phones"
  },
  {
    id: "tablets",
    label: "Tablets",
    filter: (p) =>
      /ipad|tablet/i.test(p.name) || p.categorySlug === "computers"
  },
  {
    id: "audio",
    label: "Audio",
    filter: (p) => p.categorySlug === "audio"
  },
  {
    id: "accessories",
    label: "Accessories",
    filter: (p) =>
      ["chargers", "phone-accessories", "storage"].includes(p.categorySlug)
  }
];

export function HandpickedRail({
  products,
  storeRating = null,
  reviewCount = 0
}: {
  products: Product[];
  storeRating?: number | null;
  reviewCount?: number;
}) {
  const [chipId, setChipId] = useState("all");
  const chip = CHIPS.find((c) => c.id === chipId) ?? CHIPS[0];

  const inStock = useMemo(
    () => products.filter((p) => p.stock !== "sold_out"),
    [products]
  );

  const filtered = useMemo(
    () => inStock.filter(chip.filter).slice(0, 12),
    [inStock, chip]
  );

  const deals = useMemo(
    () =>
      inStock
        .filter(
          (p) =>
            p.hotDeal || (p.compareAtPrice && p.compareAtPrice > p.price)
        )
        .slice(0, 6),
    [inStock]
  );

  const ratingLabel =
    storeRating != null
      ? `${storeRating.toFixed(1)}${reviewCount > 0 ? ` (${reviewCount})` : ""}`
      : null;

  return (
    <section className="container-g mt-10 sm:mt-12">
      <div className="text-center">
        <h2 className="display mx-auto max-w-[20ch] text-[clamp(1.25rem,0.95rem+1.4vw,1.625rem)] font-extrabold leading-snug text-gp-text sm:max-w-none">
          Handpicked for you
        </h2>
        <p className="text-subtitle mx-auto mt-2 max-w-md text-sm">
          Take what you love at present — you&apos;ll love it even more.
        </p>
      </div>

      <div className="no-scrollbar mt-5 flex gap-2 overflow-x-auto pb-1">
        {CHIPS.map((c) => {
          const active = c.id === chipId;
          return (
            <button
              key={c.id}
              type="button"
              onClick={() => setChipId(c.id)}
              className={`shrink-0 rounded-pill px-4 py-2 text-xs font-bold transition-all sm:text-sm ${
                active
                  ? "bg-ink-900 text-white shadow-sm"
                  : "border border-gp-border bg-white text-gp-text hover:border-ink-700/20"
              }`}
            >
              {c.label}
            </button>
          );
        })}
      </div>

      <div className="no-scrollbar snap-rail relative mt-5 -mx-4 flex gap-3 overflow-x-auto px-4 pb-1 sm:-mx-6 sm:gap-4 sm:px-6">
        <Link
          href="/search?deals=1"
          className="snap-item flex w-[10.5rem] shrink-0 flex-col overflow-hidden rounded-[1.25rem] border border-[#f4b942]/50 bg-gradient-to-br from-[#fff8e1] via-white to-[#e8f5e9] shadow-card transition-all hover:-translate-y-0.5 hover:shadow-card-hover sm:w-[11.5rem]"
        >
          <div className="flex flex-1 flex-col items-center justify-center px-4 py-9 text-center">
            <span className="text-3xl" aria-hidden>
              🔥
            </span>
            <p className="mt-3 font-display text-lg font-extrabold text-ink-900">
              Hot Deals
            </p>
            <p className="mt-1 text-xs font-medium text-gp-text-muted">
              Save on top picks
            </p>
          </div>
          <span className="flex items-center justify-center gap-1 border-t border-[#f4b942]/30 py-3 text-xs font-bold text-ink-700">
            Shop deals
            <Icon name="arrow-right" className="h-3.5 w-3.5" />
          </span>
        </Link>

        {filtered.map((p) => (
          <HandpickedCard
            key={p.id}
            product={p}
            ratingLabel={ratingLabel}
          />
        ))}
      </div>

      <div className="mt-8 flex justify-center">
        <span className="rounded-pill bg-gradient-to-r from-[#9b7bff] via-[#c084fc] to-[#f4a261] px-5 py-2 text-[10px] font-extrabold uppercase tracking-[0.16em] text-white shadow-sm sm:text-[11px]">
          No promo code needed
        </span>
      </div>

      {deals.length > 0 ? (
        <div className="mt-8">
          <h3 className="text-center font-display text-lg font-extrabold text-gp-text sm:text-xl">
            🔥 Featured Deals of the Week
          </h3>
          <div className="mt-5 grid grid-cols-2 gap-3 sm:gap-4">
            {deals.map((p) => (
              <FeaturedDealCard
                key={p.id}
                product={p}
                ratingLabel={ratingLabel}
              />
            ))}
          </div>
        </div>
      ) : null}
    </section>
  );
}

function productBadge(product: Product): string | null {
  if (product.hotDeal) return "Hot deal";
  if (product.featured) return "Student favorite";
  if (product.compareAtPrice && product.compareAtPrice > product.price) {
    const off = discountPercent(product.price, product.compareAtPrice);
    if (off) return `Save ${off}%`;
  }
  return null;
}

function variantSwatches(product: Product) {
  const colors = product.variants
    .filter((v) => v.colorHex)
    .slice(0, 4)
    .map((v) => v.colorHex as string);
  if (colors.length === 0) return null;
  return (
    <div className="mt-2 flex gap-1">
      {colors.map((hex, i) => (
        <span
          key={`${hex}-${i}`}
          className="h-3 w-3 rounded-full border border-black/10"
          style={{ backgroundColor: hex }}
        />
      ))}
    </div>
  );
}

function HandpickedCard({
  product,
  ratingLabel
}: {
  product: Product;
  ratingLabel: string | null;
}) {
  const variant =
    product.variants.find((v) => v.available) ?? product.variants[0] ?? null;
  const image = coverImageForProduct(product, variant);
  const priced = hasPricedOptions(product);
  const price = priced ? fromPrice(product) : product.price;
  const badge = productBadge(product);
  const compare =
    !priced && product.compareAtPrice && product.compareAtPrice > product.price
      ? product.compareAtPrice
      : null;

  return (
    <Link
      href={`/product/${product.slug}`}
      className="snap-item flex w-[10.5rem] shrink-0 flex-col overflow-hidden rounded-[1.25rem] border border-gp-border/70 bg-white shadow-card transition-all hover:-translate-y-0.5 hover:shadow-card-hover sm:w-[11.5rem]"
    >
      <div className="relative aspect-[4/5] bg-gp-muted/60">
        {image ? (
          <SafeImage
            src={image}
            alt={product.name}
            fill
            sizes="180px"
            className="object-contain p-3"
          />
        ) : null}
      </div>
      <div className="flex flex-1 flex-col p-3">
        {badge ? (
          <span className="mb-2 w-fit rounded-md bg-accent/15 px-2 py-0.5 text-[9px] font-extrabold uppercase tracking-wide text-accent-dark">
            {badge}
          </span>
        ) : null}
        <div className="flex items-center justify-between gap-1">
          {product.brand ? (
            <span className="truncate text-[10px] font-medium text-gp-text-subtle">
              {product.brand}
            </span>
          ) : (
            <span />
          )}
          {ratingLabel ? (
            <span className="shrink-0 text-[10px] font-semibold text-gp-text-muted">
              <span className="text-brand-dark">★</span> {ratingLabel}
            </span>
          ) : null}
        </div>
        <p className="mt-1 line-clamp-2 text-sm font-bold leading-snug text-gp-text">
          {product.name}
        </p>
        <div className="mt-auto flex flex-wrap items-baseline gap-x-1.5 pt-2">
          <span className="text-sm font-bold tabular-nums text-gp-text">
            {priced ? "From " : ""}
            {formatPrice(price)}
          </span>
          {compare ? (
            <span className="text-[11px] tabular-nums text-gp-text-subtle line-through">
              {formatPrice(compare)}
            </span>
          ) : null}
        </div>
        {variantSwatches(product)}
      </div>
    </Link>
  );
}

function FeaturedDealCard({
  product,
  ratingLabel
}: {
  product: Product;
  ratingLabel: string | null;
}) {
  const variant =
    product.variants.find((v) => v.available) ?? product.variants[0] ?? null;
  const image = coverImageForProduct(product, variant);
  const priced = hasPricedOptions(product);
  const price = priced ? fromPrice(product) : product.price;
  const badge = productBadge(product);
  const compare =
    !priced && product.compareAtPrice && product.compareAtPrice > product.price
      ? product.compareAtPrice
      : null;

  return (
    <Link
      href={`/product/${product.slug}`}
      className="group flex flex-col overflow-hidden rounded-[1.25rem] border border-gp-border/70 bg-white shadow-card transition-all hover:-translate-y-0.5 hover:shadow-card-hover"
    >
      <div className="relative aspect-square bg-gp-muted/60">
        {image ? (
          <SafeImage
            src={image}
            alt={product.name}
            fill
            sizes="(max-width: 640px) 46vw, 22vw"
            className="object-contain p-4 transition-transform duration-300 group-hover:scale-[1.03]"
          />
        ) : null}
      </div>
      <div className="flex flex-1 flex-col p-3.5">
        {badge ? (
          <span className="mb-2 w-fit rounded-md bg-accent/15 px-2 py-0.5 text-[9px] font-extrabold uppercase tracking-wide text-accent-dark">
            {badge}
          </span>
        ) : null}
        <div className="flex items-center justify-between gap-1">
          {product.brand ? (
            <span className="truncate text-[10px] font-medium text-gp-text-subtle">
              {product.brand}
            </span>
          ) : (
            <span />
          )}
          {ratingLabel ? (
            <span className="shrink-0 text-[10px] font-semibold text-gp-text-muted">
              <span className="text-brand-dark">★</span> {ratingLabel}
            </span>
          ) : null}
        </div>
        <p className="mt-1 line-clamp-2 text-sm font-bold leading-snug text-gp-text">
          {product.name}
        </p>
        <div className="mt-auto flex flex-wrap items-baseline gap-x-1.5 pt-2">
          <span className="text-sm font-bold tabular-nums text-gp-text">
            {priced ? "From " : ""}
            {formatPrice(price)}
          </span>
          {compare ? (
            <span className="text-[11px] tabular-nums text-gp-text-subtle line-through">
              {formatPrice(compare)}
            </span>
          ) : null}
        </div>
        {variantSwatches(product)}
      </div>
    </Link>
  );
}
