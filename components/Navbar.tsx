...
export function Navbar({ auth = null }: { auth?: ShopAuth }) {
  ...
  const minimalMobileShop =
    shopBrowseMode || isSearchFind || isProductPage || isCategoryPage;
  ...
  return (
    <header
      className={`sticky top-0 z-50 border-b border-gp-border bg-gp-surface/95 shadow-sm backdrop-blur-xl transition-shadow duration-300 ${
        scrolled ? "shadow-md" : ""
      } ${minimalMobileShop ? "hidden md:block" : ""} ${isProductPage || isCategoryPage ? "md:hidden" : ""}`}
