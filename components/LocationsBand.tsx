...
import { getLocationsWithPhotos } from "@/lib/shop-content";
...
export async function LocationsBand() {
  const defaultContact = siteConfig.contacts[0];
  const locations = await getLocationsWithPhotos().catch(() =>
    siteConfig.locations.map((loc) => ({ ...loc, photoUrls: [] as string[] }))
  );
        ...
        <div className="grid gap-4 sm:grid-cols-2 2xl:grid-cols-4">
          {locations.map((loc) => {
            ...
            return (
              <article
                key={loc.id}
                className="flex flex-col overflow-hidden rounded-[1.35rem] border border-white/[0.07] bg-ink-900/55"
              >
                {cover ? (
