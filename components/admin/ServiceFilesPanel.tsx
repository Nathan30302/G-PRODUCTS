  ...
  return (
    <DeskPanel>
      <DeskPanelHeader
        ...
        subtitle={
          files.length > 0
            ? `${files.length} file${files.length === 1 ? "" : "s"} · valid for 12 hours, then removed to free storage`
            : "No files attached to this request"
        }
      />
      {files.length === 0 ? (
        <p className="px-5 py-6 text-sm text-gp-text-subtle">
          {emptyHint ??
          ...
          {files.map((file) => (
            <li key={file.url} className="px-5 py-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div className="relative h-28 w-full shrink-0 overflow-hidden rounded-xl border border-gp-border/70 bg-gp-surface sm:h-24 sm:w-32">
                  {file.kind === "image" ? (
                    ...
                    <div className="grid h-full w-full place-items-center gap-1 text-accent-ink">
                      <Icon name="file" className="h-8 w-8" />
                      <span className="text-[10px] font-bold uppercase tracking-wider text-gp-text-subtle">
                        {kindLabel(file.kind)}
                      </span>
                    </div>
                  )}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 rounded-pill border border-gp-border px-3.5 py-2 text-xs font-semibold text-gp-text-muted hover:border-ink-700/30 hover:text-gp-text"
                    >
